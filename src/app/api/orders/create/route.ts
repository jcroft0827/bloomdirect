...
export async function POST(req: Request) {
  try {
    ...
    if (!products?.length) {
      return NextResponse.json(
        { error: "At least one product/offering is required." },
        { status: 400 },
      );
    }
    ...
    if (!sendUsage.allowed) {
      return NextResponse.json(
        {
          error:
            "You have reached the BloomFree limit of 15 sent orders this month.",
          code: "MONTHLY_SEND_LIMIT_REACHED",
          upgradeRequired: true,
          usage: {
            setThisMonth: sendUsage.sentThisMonth,
    ...
    const feeSnapshot = {
      feeType:
        originatingShopFeeOverride?.feeType ||
        originShop.financials?.feeType ||
        "flat",
      feeValue: Number(
    ...
    const order = await Order.create({
      ...
      products: products.map((p: any) => ({
        ...
        qty: Number(p.qty || 1),
        taxable: p.taxable !== false,
        pricingTierLabel: p.pricingTierLabel || "",
    ...
    const orderCreationSideEffects = await Promise.allSettled([
    ...
    for (const result of orderCreationSideEffects) {
      if (result.status === "rejected") {
        console.error(
          "Order creation side effect failed after order was saved:",
          result.reason,
        );
      }
    }

    const postCreationSideEffects = await Promise.allSettled([
