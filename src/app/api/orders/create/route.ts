...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.log("CREATE PAYLOAD:", JSON.stringify(body, null, 2)); // ← DEBUG
    ...
    const [originShop, fulfillShop] = await Promise.all([
      Shop.findById(session.user.shopId),
    ...
    const order = await Order.create({
      ...
      originatingShop: session.user.shopId,
    ...
    console.log("ORDER SAVED:", order._id); // ← DEBUG
