  ...
  try {
    ...
    if (!shop.isPro) {
      return NextResponse.json(
        { error: "Only pro shops can enable API access." },
        { status: 403 },
      );
    }

    if (!shop.apiAccess?.keyCreatedAt || !shop.apiAccess?.keyHash) {
      return NextResponse.json(
    ...
    console.error("Enable API access error:", error);

    return NextResponse.json(
      { error: "Failed to enable API access." },
      { status: 500 },
    );
  }
}