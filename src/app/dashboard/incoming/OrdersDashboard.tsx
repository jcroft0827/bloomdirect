...
export default function OrdersDashboard() {
  ...
  const [activeFilters, setActiveFilters] = useState({
    ...
    refundStatus: "all" as "all" | "none" | "partial" | "full",
    dateRange: { start: "", end: "" },
    dateType: "Order Date",
    preset: "",
  });
  ...
  const fetchOrders = async () => {
    try {
      ...
      const { status, role, refundStatus, dateRange, dateType } = activeFilters;
  ...
  const handleStatus = async (orderId: string, newStatus: OrderStatus) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Order updated: ${getOrderStatusLabel(newStatus)}`);
                ...
                <div className="pt-4 border-t border-gray-200 mt-2 text-gray-600 text-sm space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-y-2 gap-x-4 flex-wrap">
                      {filterableStatuses.map((filterStatus) => (
                        <label
                          key={filterStatus}
                        ...
                        >
                          <input
                            ...
                            checked={draftFilters.status.includes(filterStatus)}
                            onChange={() => {
                              const isSelected =
                                draftFilters.status.includes(filterStatus);
                              const nextStatus = isSelected
                                ? draftFilters.status.filter(
                                    (s) => s !== filterStatus,
                                  ) // Remove if already there
                                : [...draftFilters.status, filterStatus]; // Add if not there
                          ...
                          <span className="text-sm text-gray-700">
                            {getOrderStatusLabel(filterStatus)}
                          </span>
                        </label>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center gap-y-2 gap-x-4 flex-wrap">
                      {[
                        ...
                        >
                          <input
                            type="radio"
                            name="desktopRefundStatus"
                            value={option.value}
                            checked={draftFilters.refundStatus === option.value}
                            onChange={() =>
                              setDraftFilters({
                                ...draftFilters,
                                refundStatus: option.value as
                                  | "all"
                                  | "none"
                                  | "partial"
                                  | "full",
          ...
          {orders.length === 0 ? (
            ...
            <div className="space-y-12">
              {orders.map((order) => (
                <div
                  key={order._id}
                  className={`bg-white rounded-3xl shadow-2xl overflow-hidden border-l-8 transition ${
                  ...
                  <div className="grid lg:grid-cols-2 gap-8 p-4 lg:py-6 2xl:grid-cols-3 2xl:gap-4">
                    ...
                    <div className="space-y-4 text-center lg:col-span-1">
                      ...
                      <div className="flex flex-col items-center justify-center space-y-4">
                        ...
                        <span
                          className={`inline-flex rounded-full px-3 py-1 text-sm font-semibold ${getOrderStatusBadgeClass(
